...
class Dashboard extends React.Component {
  ...
  componentDidMount() {
    const targetUrl = "https://www.atlageek.com/api/getokresults";
    trackPromise(
      fetch(targetUrl)
    ...
    const targetUrl2 = "https://www.atlageek.com/api/getbadresults";
    trackPromise(
      fetch(targetUrl2)
        .then((response) => response.json())
        .then((data) =>
    ...
    if (fullData) {
      var listItems = fullData.map((d) => (
        <tr>
          ...
          <td>
            <span> {d.PageTitle.substr(0, 29)} </span>
          </td>
          <td className="text-center">
      ...
      var listItemsErrors = errorsData.map((d) => (
        <tr>
          ...
          <td className="text-center ">
            ...
            >
              {d.keyword === undefined ? "OLD SHEMA" : d.keyword.substr(0, 29)}
            </strong>
          </td>
          <td className="text-center ">
            <strong
            ...
            >
              {d.website === undefined ? "OLD SHEMA" : d.website.substr(0, 33)}
            </strong>
          </td>
          <td>
